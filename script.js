...
morseInput.addEventListener("input", () => {
  ...
  const errorMorse = document.getElementById("errorMorse");

  // Validasi: hanya boleh . - / dan spasi
  const valid = /^[.\-/\s]*$/;

  if (!valid.test(morse)) {
    errorMorse.classList.remove("d-none");
    textOutput.textContent = "";
    return;
  }

  errorMorse.classList.add("d-none");
...
const translations = {
  en: {
    ...
    errorMorse: "Only dots (.), dashes (-), slashes (/), and spaces are allowed.",
    ...
    update_available: "Versi baru tersedia. Muat Ulang"
  },
  id: {
    ...
    errorMorse: "Hanya titik (.), strip (-), garis miring (/), dan spasi saja yang diperbolehkan.",
    ...
    update_available: "New version is ready. Reload"
  }
};
